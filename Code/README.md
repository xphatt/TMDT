...
- Liên hệ/feedback và đánh giá sản phẩm được validate ở Client lẫn Server, lưu D1 và có màn quản trị riêng.
- Catalogue hỗ trợ khuyến mãi theo thời gian, giá cũ/giá mới; backend luôn tính lại giá hiện hành khi tạo đơn.
- Ba trang chính sách dành cho đồ án được liên kết từ footer và checkout.
- Thông tin cửa hàng/Google Maps chỉ bật khi `STORE_ADDRESS` đã được xác nhận; project không bịa địa chỉ.
...
app/
  ...
  api/feedback/             Lưu góp ý có validation và idempotency
  api/reviews/              Đánh giá sản phẩm và quyền sở hữu nội bộ
  api/store-info/           Địa chỉ/Google Maps từ cấu hình
  api/health/               Health check D1 cho triển khai
  ...
  server/catalogue/       Catalogue/promotion D1 với dữ liệu fallback
  server/engagement/      Feedback, review, ownership và moderation
...
docs/completion/           Kế hoạch, ma trận và kết quả hoàn thiện
docs/demo/                 Kịch bản lỗi checkout development-only
docs/deployment/           Kế hoạch Cloudflare Workers + D1, chưa deploy
```
...
- `GET|POST /api/admin/catalogue`
- `PATCH|DELETE /api/admin/catalogue/products/:id`
- `GET|POST /api/admin/promotions`
- `PATCH|DELETE /api/admin/promotions/:id`
- `GET /api/admin/feedback` và `PATCH /api/admin/feedback/:id`
- `GET /api/admin/reviews` và `PATCH|DELETE /api/admin/reviews/:id`
...
Mặc định demo tắt. Để trình diễn một lỗi 503 an toàn trong development:
...
Lần gửi đầu của một request ID hiển thị lỗi và nút **Thử lại**; lần retry dùng cùng ID để tạo đúng một đơn. Production build luôn bỏ qua cờ demo. Xem [`docs/demo/DEMO_ERROR_SCENARIO.md`](docs/demo/DEMO_ERROR_SCENARIO.md).
...
Dự án chỉ được triển khai local trong lần thay đổi này. Chưa tạo D1 production, tài khoản admin production, hostname public hoặc DNS. Trạng thái là **SOURCE READY FOR DEPLOYMENT**, không phải `DEPLOYED`. Xem [`docs/deployment/PUBLIC_DEPLOYMENT_PLAN.md`](docs/deployment/PUBLIC_DEPLOYMENT_PLAN.md) trước khi chọn Cloudflare và cấp quyền triển khai.
