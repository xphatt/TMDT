...
type View = "home" | "menu" | "product" | "cart" | "checkout" | "success" | "contact";
...
function productById(id: string, catalogue = staticProducts) {
  return catalogue.find((product) => product.id === id) ?? staticProducts.find((product) => product.id === id);
}

function ProductPrice({ product, totalMultiplier = 1 }: { product: Product; totalMultiplier?: number }) {
  const current = product.price * totalMultiplier;
  const original = product.originalPrice ? product.originalPrice * totalMultiplier : null;
  return <span className="price-display"><strong>{formatVnd(current)}</strong>{original && original > current ? <del>{formatVnd(original)}</del> : null}</span>;
}
  ...
  return (
    <header className="site-header">
      ...
      <nav className="desktop-nav" aria-label="Điều hướng chính">
        ...
        <button type="button" onClick={() => navigate("contact")}>Liên hệ</button>
      </nav>
...
function HomeView({ catalogueProducts, catalogueCategories, onOpenMenu, onSelectProduct }: { catalogueProducts: Product[]; catalogueCategories: typeof staticCategories; onOpenMenu: (category?: CategoryId) => void; onSelectProduct: (product: Product) => void }) {
  const bestSellers = [...catalogueProducts].filter((product) => product.tags.includes("Bán chạy") && product.category !== "topping").slice(0, 3);
  return (
...
function MenuView({ catalogueProducts, catalogueCategories, initialCategory, onSelectProduct }: { catalogueProducts: Product[]; catalogueCategories: typeof staticCategories; initialCategory: "all" | CategoryId; onSelectProduct: (product: Product) => void }) {
  ...
  }, [catalogueCategories, catalogueProducts, category, query, sort]);
    ...
    <main id="main-content" className="page-main">
      ...
      <section className="catalogue" aria-labelledby="catalogue-heading">
        ...
        <div className="filter-chips" aria-label="Lọc theo danh mục">
          {catalogueCategories.map((item) => <button key={item.id} className={category === item.id ? "active" : ""} type="button" aria-pressed={category === item.id} onClick={() => setCategory(item.id)}>{item.label}</button>)}
        </div>
...
function ProductView({ product, catalogueToppings, onBack, onAdd }: { product: Product; catalogueToppings: ToppingOption[]; onBack: () => void; onAdd: (item: CartItem) => void }) {
  ...
  const toppingTotal = selectedToppings.reduce((sum, id) => sum + (catalogueToppings.find((item) => item.id === id)?.price ?? 0), 0);
    ...
    <main id="main-content" className="page-main product-page">
      ...
      <div className="product-detail">
        ...
        <div className="product-config">
          ...
          {product.promotion && <p className="promotion-label">{product.promotion.label}</p>}
          <p className="product-price" aria-live="polite"><ProductPrice product={{ ...product, price: unitPrice, originalPrice: product.originalPrice ? product.originalPrice + (size ? sizeSurcharge[size] : 0) + toppingTotal : undefined }} totalMultiplier={quantity} /></p>
          ...
          {!isTopping && (
            <>
              ...
              <fieldset className="option-group topping-group"><legend>Thêm topping</legend>{catalogueToppings.map((item) => <label key={item.id}><input type="checkbox" checked={selectedToppings.includes(item.id)} onChange={() => toggleTopping(item.id)} /><span>{item.name}</span><small>+{formatVnd(item.price)}</small></label>)}</fieldset>
            </>
          )}
...
function CartView({ items, catalogueProducts, catalogueToppings, onUpdate, onRemove, onMenu, onCheckout }: { items: CartItem[]; catalogueProducts: Product[]; catalogueToppings: ToppingOption[]; onUpdate: (key: string, quantity: number) => void; onRemove: (key: string) => void; onMenu: () => void; onCheckout: () => void }) {
    ...
    <main id="main-content" className="page-main cart-page">
      ...
      <div className="cart-layout">
        <section className="cart-list" aria-label="Sản phẩm trong giỏ">{items.map((item) => {
          const product = productById(item.productId, catalogueProducts); if (!product) return null;
          const toppingNames = item.toppings.map((id) => catalogueToppings.find((topping) => topping.id === id)?.name).filter(Boolean).join(", ");
...
function CheckoutView({ items, catalogueProducts, onBack, onSuccess }: { items: CartItem[]; catalogueProducts: Product[]; onBack: () => void; onSuccess: (order: MockOrder) => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!orderId) {
        const pendingOrder = await createOrder(details, items, requestIdRef.current);
  ...
  return (
    <main id="main-content" className="page-main checkout-page">
      ...
      <form ref={formRef} className="checkout-layout" onSubmit={submit} noValidate>
        <section className="checkout-form" aria-label="Thông tin giao hàng">
          ...
          {submitError && <div className="validation-summary" id="checkout-submit-error" tabIndex={-1} role="alert"><strong>Chưa thể xác nhận đơn</strong><p>{submitError}</p><p>Giỏ hàng và thông tin đã nhập vẫn được giữ lại.</p><button className="button button-secondary retry-button" type="button" disabled={submitting} onClick={() => formRef.current?.requestSubmit()}>Thử lại</button></div>}
        ...
        <aside className="order-summary checkout-summary"><h2>Đơn của bạn</h2>{items.map((item) => <div className="checkout-line" key={item.key}><span>{item.quantity} x {productById(item.productId, catalogueProducts)?.name}</span><strong>{formatVnd(item.unitPrice * item.quantity)}</strong></div>)}<div><span>Tiền món</span><strong>{formatVnd(subtotal)}</strong></div><div><span>Phí giao hàng mẫu</span><strong>{formatVnd(deliveryFee)}</strong></div><div className="summary-total"><span>Tổng cộng</span><strong>{formatVnd(total)}</strong></div><button className="button button-primary" type="submit" disabled={submitting}>{submitting ? (pendingOrderId ? "Đang xác nhận đơn" : "Đang tạo đơn") : `Xác nhận đơn - ${formatVnd(total)}`}</button><p className="simulation-note">Đơn được tạo ở trạng thái chờ và chỉ xác nhận nội bộ. Không có khoản tiền nào được thu trong bản demo này.</p><p className="checkout-policy-links"><a href="/chinh-sach/van-chuyen">Vận chuyển</a><a href="/chinh-sach/doi-tra">Đổi trả</a><a href="/chinh-sach/bao-hanh">Bảo đảm chất lượng</a></p></aside>
      </form>
    </main>
  );
}
...
function Footer({ onNavigate }: { onNavigate: (view: View) => void }) {
  return <footer className="site-footer"><div><button className="footer-wordmark" type="button" onClick={() => onNavigate("home")}>Trà Sữa Ngon</button><p>Website thương mại điện tử mẫu cho thương hiệu hư cấu.</p></div><div><h2>Khám phá</h2><button type="button" onClick={() => onNavigate("menu")}>Thực đơn</button><button type="button" onClick={() => onNavigate("cart")}>Giỏ hàng</button><button type="button" onClick={() => onNavigate("contact")}>Liên hệ</button></div><div><h2>Chính sách</h2><a href="/chinh-sach/doi-tra">Đổi trả</a><a href="/chinh-sach/bao-hanh">Bảo đảm chất lượng</a><a href="/chinh-sach/van-chuyen">Vận chuyển</a></div><p className="footer-bottom">© 2026 Trà Sữa Ngon. Thanh toán chỉ là mô phỏng.</p></footer>;
}
...
export function TeaShop() {
  ...
  const [catalogueProducts, setCatalogueProducts] = useState<Product[]>(staticProducts);
  const [catalogueCategories, setCatalogueCategories] = useState<typeof staticCategories>(staticCategories);
  const [catalogueToppings, setCatalogueToppings] = useState<ToppingOption[]>(staticToppings);
  ...
  useEffect(() => {
    const controller = new AbortController();
    fetch("/api/menu", { signal: controller.signal, headers: { accept: "application/json" } })
      .then((response) => response.ok ? response.json() : Promise.reject(new Error("menu unavailable")))
      .then((payload: { products?: Product[]; categories?: typeof staticCategories; toppings?: ToppingOption[] }) => {
        if (payload.products?.length) setCatalogueProducts(payload.products);
        if (payload.categories?.length) setCatalogueCategories(payload.categories);
        if (payload.toppings?.length) setCatalogueToppings(payload.toppings);
      })
      .catch(() => { if (!controller.signal.aborted) setStorageError("Không thể đồng bộ thực đơn mới. Website đang dùng dữ liệu mẫu có sẵn."); });
  ...
  const addToCart = (item: CartItem) => {
    setCart((current) => {
      const existing = current.find((cartItem) => cartItem.key === item.key);
       return existing ? current.map((cartItem) => cartItem.key === item.key ? { ...cartItem, quantity: Math.min(20, cartItem.quantity + item.quantity) } : cartItem) : [...current, item];
    });
    setToast(`${productById(item.productId, catalogueProducts)?.name ?? "Món"} đã vào giỏ hàng.`);
    setView("cart");
  };
  ...
  return (
    <div className="site-shell">
      ...
      {view === "home" && <HomeView catalogueProducts={catalogueProducts} catalogueCategories={catalogueCategories} onOpenMenu={openMenu} onSelectProduct={openProduct} />}
      {view === "menu" && <MenuView catalogueProducts={catalogueProducts} catalogueCategories={catalogueCategories} initialCategory={initialCategory} onSelectProduct={openProduct} />}
      {view === "product" && selectedProduct && <ProductView product={selectedProduct} catalogueToppings={catalogueToppings} onBack={() => setView("menu")} onAdd={addToCart} />}
      {view === "cart" && <CartView items={cart} catalogueProducts={catalogueProducts} catalogueToppings={catalogueToppings} onUpdate={updateQuantity} onRemove={(key) => setCart((current) => current.filter((item) => item.key !== key))} onMenu={() => openMenu()} onCheckout={() => navigate("checkout")} />}
      {view === "checkout" && <CheckoutView items={cart} catalogueProducts={catalogueProducts} onBack={() => setView("cart")} onSuccess={completeOrder} />}
