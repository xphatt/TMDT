...
export async function createOrder(customer: CheckoutDetails, items: CartItem[], clientRequestId: string): Promise<ApiOrder> {
  const response = await fetch("/api/orders", {
    method: "POST",
    headers: { "content-type": "application/json", accept: "application/json" },
    body: JSON.stringify({ customer, items, clientRequestId }),
  });
