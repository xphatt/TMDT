...
export class D1OrderRepository implements OrderRepository {
  ...
  async create(order: OrderRecord): Promise<OrderRecord> {
    const statements: RuntimeD1PreparedStatement[] = [
      this.database.prepare(
        `INSERT INTO orders (
          ...
          confirmed_at, created_at, updated_at, version, idempotency_key
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ).bind(
        order.id,
        order.id,
    ...
    for (const item of order.items) {
      ...
      for (const toppingId of item.toppings) {
        const topping = item.toppingDetails?.find((entry) => entry.id === toppingId);
  ...
  async findById(id: string): Promise<OrderRecord | null> {
    const orderRow = await this.database.prepare(
      `SELECT id, idempotency_key, customer_name, phone, delivery_address, note, subtotal,
    ...
    const [itemResult, toppingResult, paymentRow] = await Promise.all([
      this.database.prepare(
        `SELECT id, product_id, product_name, unit_price, size, sugar, ice, quantity
    ...
    const items = itemResult.results.map((row): CartItem => {
      ...
      return {
        ...
        toppingDetails: toppingResult.results.filter((entry) => entry.order_item_id === row.id).map((entry) => ({ id: entry.topping_id, name: entry.topping_name, price: entry.unit_price })),
      };
    });
  ...
  async findByIdempotencyKey(key: string): Promise<OrderRecord | null> {
    const row = await this.database.prepare("SELECT id FROM orders WHERE idempotency_key = ?").bind(key).first<{ id: string }>();
