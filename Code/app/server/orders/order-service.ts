...
function normalizeItems(value: unknown, products: Awaited<ReturnType<typeof getCatalogueSnapshot>>["products"], toppings: Awaited<ReturnType<typeof getCatalogueSnapshot>>["toppings"]): CartItem[] {
  ...
  return value.map((raw, index) => {
    ...
    return {
      ...
      toppingDetails: selectedToppings.filter((item): item is NonNullable<typeof item> => Boolean(item)).map((item) => ({ id: item.id, name: item.name, price: item.price })),
    };
  });
}

export async function createPendingOrder(payload: unknown): Promise<OrderRecord> {
  const input = payload && typeof payload === "object" ? payload as { customer?: unknown; items?: unknown; clientRequestId?: unknown } : {};
  ...
  const items = normalizeItems(input.items, catalogue.products, catalogue.toppings);
  const totals = calculateOrderTotals(items.map((item) => item.unitPrice * item.quantity), deliveryFee);
