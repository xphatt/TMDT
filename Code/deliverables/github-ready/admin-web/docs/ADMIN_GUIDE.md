...
Hai thư mục phải nằm cạnh nhau để cùng dùng D1 local tại `../.tra-sua-ngon-local-state/`.
...
Không cần biến chứa mật khẩu admin. Tài khoản và session nằm trong D1. Production phải đặt `SITE_URL` thành origin web quản lý và `STOREFRONT_URL` thành origin cửa hàng.
...
`npm test` build production và chạy smoke test với D1 tạm thời bằng runtime Cloudflare local. Test kiểm tra redirect, bảo vệ route/API, trang đăng nhập và robots; bộ integration đầy đủ cho auth, state transition, COD và audit vẫn nên chạy trong pipeline tích hợp hai web.
...
4. Chính sách retention PII/audit và quyền truy cập dữ liệu.
