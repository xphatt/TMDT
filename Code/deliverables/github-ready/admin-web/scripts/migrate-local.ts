...
async function main(): Promise<void> {
  ...
  const localStateDirectory = join(process.cwd(), "..", ".tra-sua-ngon-local-state");
  await mkdir(wranglerRuntimeDirectory, { recursive: true });
  await mkdir(localStateDirectory, { recursive: true });
  await runWrangler(["d1", "migrations", "apply", "DB", "--local", "--persist-to", localStateDirectory], wranglerRuntimeDirectory);
  await runWrangler(["d1", "execute", "DB", "--local", "--persist-to", localStateDirectory, "--command", "PRAGMA optimize;"], wranglerRuntimeDirectory);
}
