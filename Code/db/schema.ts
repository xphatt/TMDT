...
export const catalogueCategories = sqliteTable("catalogue_categories", {
  id: text("id").primaryKey(),
...
export const catalogueProducts = sqliteTable("catalogue_products", {
  id: text("id").primaryKey(),
  ...
  imagePosition: text("image_position").notNull().default("50% 50%"),
  tone: text("tone").notNull().default("milk"),
  isActive: integer("is_active", { mode: "boolean" }).notNull().default(true),
  createdAt: text("created_at").notNull(),
  updatedAt: text("updated_at").notNull(),
}, (table) => [
  uniqueIndex("idx_catalogue_products_slug").on(table.slug),
  index("idx_catalogue_products_category").on(table.categoryId),
]);

export const catalogueToppings = sqliteTable("catalogue_toppings", {
  id: text("id").primaryKey(),
  name: text("name").notNull(),
...
export const promotions = sqliteTable("promotions", {
  ...
  index("idx_promotions_product").on(table.productId),
  index("idx_promotions_window").on(table.isActive, table.startsAt, table.endsAt),
]);
...
export const feedback = sqliteTable("feedback", {
  ...
  uniqueIndex("idx_feedback_client_request").on(table.clientRequestId),
...
export const reviews = sqliteTable("reviews", {
  ...
  uniqueIndex("idx_reviews_client_request").on(table.clientRequestId),
  uniqueIndex("idx_reviews_owner_product").on(table.ownerTokenHash, table.productId),
  index("idx_reviews_product_status").on(table.productId, table.status, table.createdAt),
]);
